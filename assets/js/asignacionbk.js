...
function facultades() {
 ...
 $.ajax({
      type: 'GET',
      url:  dominio + "buscarFacultad/" + idCentro,
 ...
 $.ajax({
    ...
    //  console.log(data.contador[0].count);
      ...
      if(data.contador[0].count <= cupo){
        ...
        buscarResultado(idMateria, idDetalleSalon, novCarne, data.contador[0].count, fechaExamen);
...
function buscarResultado(idMateria, idDetalleSalon, novOCarne, contadorAsignado, fechaExamen){
/*
novResultado = 0;
if(novCarne.length === 10){
  novResultado = novCarne;
...
*/
 $.ajax({
      type: 'GET',
      url:  dominio + "resultadoPorAsignacion/" + novOCarne + "/" + idMateria + "/1",
      contentType: "application/json",
      dataType: 'json',
      async: false,
      success: function (data) {
